...
const string usage = "\n"
  ...
  "  -f <fy>         Focal length in pixels\n"
  "  -u <cx>         principle point x direction\n"
  "  -v <cy>         principal point y direction\n"
  "  -R <d1>         distortion:[d1, d2, k1, k2]\n"
  "  -r <d2>         distortion:[d1, d2, k1, k2]\n"
  "  -K <k1>         distortion:[d1, d2, k1, k2]\n"
  "  -k <k2>         distortion:[d1, d2, k1, k2]\n"
...
const char* window_name_right = "apriltags_demo_right";
...
public:
  ...
  Demo() :
    ...
    m_px(m_width/2),
    m_py(m_height/2),
    m_distortion(cv::Mat(cv::Size(1, 4), CV_64F)),
  ...
  void parseOptions(int argc, char* argv[]) {
    int c;
    while ((c = getopt(argc, argv, ":h?adC:F:f:u:v:H:S:W:E:G:B:R:r:K:k:")) != -1) {
      ...
      case 'v':
        ...
        break;
      case 'R':
        m_distortion.at<double>(0, 0)= atof(optarg);
        break;
      case 'r':
        m_distortion.at<double>(1, 0)= atof(optarg);
        break;
      case 'K':
        m_distortion.at<double>(2, 0)= atof(optarg);
        break;
      case 'k':
        m_distortion.at<double>(3, 0)= atof(optarg);
        break;
      case 'H':
  ...
  void setup() {
    ...
    if (m_draw) {
      ...
      cv::namedWindow(window_name_right, 1);
    }
  ...
  void print_detection(AprilTags::TagDetection& detection) const {
   ...
   //      << " (Hamming: " << detection.hammingDistance << ")";
    ...
    //std::cout << "m_tagSize: " << m_tagSize << std::endl;
    //std::cout << "m_fx: " << m_fx << std::endl;
    //std::cout << "m_fy: " << m_fy << std::endl;
    //std::cout << "m_distortion: " << m_distortion.t() << std::endl;
    detection.getRelativeTranslationRotation(m_tagSize, m_fx, m_fy, m_px, m_py, m_distortion,
 ...
 //   cout << "  distance=" << translation.norm() << "\n";
  ...
  void loop() {
    ...
    while (true) {
      ...
      // capture frame
      m_cap >> lr_img;
      image = lr_img(cv::Rect(0, 0, lr_img.cols/2, lr_img.rows));
      image_r = lr_img(cv::Rect(lr_img.cols/2, 0, lr_img.cols/2, lr_img.rows));
      ...
      cv::cvtColor(image_r, image_gray_r, CV_BGR2GRAY);
      vector<AprilTags::TagDetection> detections = m_tagDetector->extractTags(image_gray);
      vector<AprilTags::TagDetection> detections_r = m_tagDetector->extractTags(image_gray_r);
      // print out each detection
      //cout << detections.size() << " tags detected (left):" << endl;
      //cout << detections_r.size() << " tags detected(right):" << endl;
      ...
      //                                                 m_distortion, translation, rotation);
     ...
     // }
      ...
      if (m_draw) {
        ...
        for (int i=0; i<detections_r.size(); i++) {
          // also highlight in the image
          detections_r[i].draw(image_r);
        }
        imshow(window_name, image); // OpenCV call
        imshow(window_name_right, image_r); // OpenCV call
      }
