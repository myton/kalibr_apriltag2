...
Eigen::Matrix4d TagDetection::getRelativeTransform(double tag_size, double fx, double fy, double px, double py, const cv::Mat& D) const {
  CV_Assert(D.depth() == CV_64F || D.total() == 4);
...
//  cv::Vec4f distParam(0,0,0,0); // all 0?
  ...
  bool flg = cv::solvePnP(objPts, imgPts, cameraMatrix, D, rvec, tvec);
...
void TagDetection::getRelativeTranslationRotation(double tag_size, double fx, double fy, double px, double py, const cv::Mat& D,
                                                  Eigen::Vector3d& trans, Eigen::Matrix3d& rot) const {
  Eigen::Matrix4d T =
    getRelativeTransform(tag_size, fx, fy, px, py, D);
